"""Render an existing daily run as a 9:16 vertical video without rerunning providers.

This module is intentionally standalone from the scheduled daily pipeline. It reads the
already-generated ``report.json``, ``narration.mp3`` and ``narration_timing.json`` from
``output/<date>/`` and writes portrait-only artifacts beneath ``output/<date>/vertical/``.
No rankings, match, LLM, voice, or YouTube API is called.

Usage::

    python -m wta_daily.vertical --config config/config.yaml --date 2026-09-05
"""
...
from wta_daily.graphics.utils import (
    fit_text,
    hex_to_rgb,
    movement_color,
    movement_headline_text,
)
...
def _portrait_config(source: GraphicsConfig) -> GraphicsConfig:
    """Use the production theme/fonts with a 1080x1920 canvas."""
...
def _draw_header(
    ...
    *,
    ...
    title_font = load_font(theme.font_bold, size=int(header_h * 0.30), bold=True)
    subtitle_font = load_font(theme.font_regular, size=int(header_h * 0.16))
    x = int(width * 0.055)
    draw.text((x, int(header_h * 0.20)), title, font=title_font, fill=text_color, anchor="la")
    draw.text(
        (x, int(header_h * 0.66)),
        subtitle,
        font=subtitle_font,
    ...
    if right_text:
        ...
        draw.text(
            (int(width * 0.945), int(header_h * 0.72)),
            right_text,
            font=right_font,
            fill=accent,
            anchor="ra",
...
def render_vertical_leaderboard(report: DailyReport, output_path: Path, graphics: GraphicsConfig) -> Path:
    ...
    header_h = _draw_header(
        ...
        theme=theme,
        title=f"{profile.display_name} TOP {len(report.players)}",
        subtitle="OFFICIAL RANKINGS",
        right_text=report.report_date.strftime("%b %d").upper(),
    )
    ...
    for index, player in enumerate(report.players):
        ...
        draw.text(
            (points_right, y1 + row_h * 0.62),
            "POINTS",
            font=points_label_font,
            fill=subtext_color,
            anchor="ra",
        )
        move = movement_headline_text(
            player.movement,
            rank=player.rank,
        ...
        draw.text(
            (points_right, y1 + row_h * 0.80),
            move,
            font=move_font,
            fill=movement_color(player.movement, theme),
            anchor="ra",
        )

    footer_font = load_font(theme.font_regular, size=int(footer_h * 0.30))
    draw.text(
...
def _render_person_card(
    *,
    ...
    name_font = load_font(theme.font_bold, size=int(height * 0.044), bold=True)
    ...
    if featured_label:
        ...
        draw.rounded_rectangle(
            [margin, y, margin + label_w, y + label_h],
            radius=int(label_h * 0.45),
            fill=accent,
        )
        draw.text(
            (margin + label_w / 2, y + label_h / 2),
            featured_label,
            font=kicker_font,
            fill=bg,
            anchor="mm",
    ...
    fitted_name = fit_text(draw, name, name_font, max_name_w)
    draw.text((name_x, y), fitted_name, font=name_font, fill=text_color, anchor="la")
    name_bbox = draw.textbbox((name_x, y), fitted_name, font=name_font, anchor="la")
    ...
    draw.rounded_rectangle(
        [margin, metrics_top, width - margin, int(height * 0.51)],
        radius=int(height * 0.018),
        fill=panel,
    )
    points_value = f"{points:,}" if points is not None else "—"
    draw.text(
        (margin * 1.45, metrics_top + height * 0.045),
        points_value,
        font=points_font,
        fill=text_color,
        anchor="la",
    ...
    draw.text(
        (margin * 1.45, metrics_top + height * 0.12),
        "RANKING POINTS",
        font=points_label_font,
        fill=subtext_color,
        anchor="la",
    ...
    if movement is not None and rank is not None:
        movement_text = movement_headline_text(
            movement, rank=rank, top_n=top_n, previous_rank=previous_rank
        )
        ...
        draw.rounded_rectangle(
            [badge_x1, badge_y1, badge_x2, badge_y1 + badge_h],
            radius=int(badge_h * 0.35),
            outline=badge_color,
            width=3,
        )
        draw.text(
            ((badge_x1 + badge_x2) / 2, badge_y1 + badge_h / 2),
            movement_text,
            font=badge_font,
            fill=badge_color,
            anchor="mm",
    ...
    match_top = int(height * 0.56)
    match_bottom = int(height * 0.90)
    draw.rounded_rectangle(
        [margin, match_top, width - margin, match_bottom],
        radius=int(height * 0.02),
        fill=panel,
    )
    section_label = "YESTERDAY'S MATCH" if supports_daily_matches else "RANKING UPDATE"
    draw.text(
        (margin * 1.45, match_top + height * 0.045),
        section_label,
        font=section_font,
        fill=subtext_color,
        anchor="la",
    ...
    if match is not None:
        ...
        opponent_line = fit_text(
            draw,
            f"{outcome} vs {match.opponent}",
            result_font,
            int(width - margin * 3),
        )
        draw.text(
            (margin * 1.45, match_top + height * 0.105),
            opponent_line,
            font=result_font,
            fill=outcome_color,
            anchor="la",
        ...
        draw.text(
            (margin * 1.45, match_top + height * 0.18),
            score_line,
            font=result_font,
            fill=text_color,
            anchor="la",
        ...
        draw.text(
            (margin * 1.45, match_top + height * 0.245),
            tournament_line,
            font=detail_font,
            fill=text_color,
            anchor="la",
        ...
        date_label = (
            f"{match.match_date:%B} {match.match_date.day}, {match.match_date.year}"
            if match.match_date
            else "Date unconfirmed"
        )
        draw.text(
            (margin * 1.45, match_top + height * 0.295),
            date_label,
            font=detail_font,
            fill=subtext_color,
            anchor="la",
        ...
        draw.text(
            (margin * 1.45, match_top + height * 0.12),
            message,
            font=result_font,
            fill=subtext_color,
            anchor="la",
...
def render_vertical_player_card(
    player: PlayerReport,
    output_path: Path,
    graphics: GraphicsConfig,
    *,
    ...
    return _render_person_card(
        ...
        previous_rank=player.previous_rank,
        match=player.match,
        match_error=player.match_error,
...
def render_vertical_featured_card(
    ...
    *,
    ...
    return _render_person_card(
        ...
        movement=featured.movement,
        previous_rank=featured.previous_rank,
        match=featured.match,
        match_error=featured.match_error,
        output_path=output_path,
        graphics=graphics,
        top_n=top_n,
        supports_daily_matches=supports_daily_matches,
        featured_label="FEATURED PLAYER",
    )
...
def assemble_vertical_video(
    *,
    ...
    if not segments:
        raise RuntimeError(
            f"Missing or unusable narration timing: {source_store.timing_path}. "
            "Vertical rerender intentionally requires the completed run's timing file."
        )

    slides = _merge_consecutive_identical_images(
        [
            (
                _image_for_segment(segment, vertical_root),
                max(segment.duration_seconds, _MIN_SLIDE_SECONDS),
            )
    ...
    build = [
        ...
        str(concat_path),
        "-vf",
        f"scale={PORTRAIT_WIDTH}:{PORTRAIT_HEIGHT},fps={fps}",
        "-pix_fmt",
        "yuv420p",
        "-c:v",
        "libx264",
    ...
    mux = [
        ...
        str(silent_path),
        "-i",
        str(source_store.narration_path),
        "-c:v",
        "copy",
        "-c:a",
        "aac",
...
def rerender_vertical(config_path: str | Path, report_date: date) -> Path:
    """Create portrait graphics/video from an already-completed run."""

    config = load_config(config_path)
    source_store = DailyOutputStore(config.output_dir, report_date)
    if not source_store.report_path.exists():
        raise FileNotFoundError(
            f"No completed report found at {source_store.report_path}; run the normal pipeline first."
        )
    ...
    for player in report.players:
        render_vertical_player_card(
            player,
            cards_dir / f"{player.rank:02d}.png",
            graphics,
            top_n=config.top_n,
            supports_daily_matches=profile.supports_daily_matches,
        )
    if report.featured_player is not None:
        render_vertical_featured_card(
            report.featured_player,
            vertical_root / "featured_player.png",
            graphics,
            top_n=config.top_n,
            supports_daily_matches=profile.supports_daily_matches,
        )

    output_path = assemble_vertical_video(
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="wta-daily-vertical",
        description="Rerender an existing completed WTA/ATP run as a 1080x1920 vertical video.",
    )
    parser.add_argument(
        "--config",
        default="config/config.yaml",
        help="Path to the existing YAML configuration file.",
    )
    parser.add_argument(
        "--date",
        required=True,
        help="Completed report date to rerender (YYYY-MM-DD).",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    except Exception as exc:  # noqa: BLE001 - standalone CLI boundary
